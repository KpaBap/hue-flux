...
def discover_and_connect_bridge():
    ...
    while not(registered):
        try:
            ...
            time.sleep(1)
            if not help_printed:
                print("Please press the button on the Hue bridge to connect this app.")
                help_printed = True
...
def verify_hue_upnp(url):
    ...
    hub_config = urllib.request.urlopen(baseurl+"api/%20/config").read().decode()
...
def find_computer_lights(bridge):
    lights = bridge.get_light_objects()
    computer_lights = []

    for light in lights:
        if light.name.find("Computer") != -1:
...
def fade_colortemp_k(lights, temp_from, temp_to, fadetime):
    ...
    # If the color temp steps work out to less than 1, increase the delay instead
...
# Taken from: http://www.developers.meethue.com/documentation/color-conversions-rgb-xy
def RGB_to_xy(red, green, blue):
    ...
    if cx > 0.675: cx = 0.675
    if cx < 0.167: cx = 0.167

    if cy > 0.518: cy = 0.518
    if cy < 0.04: cy = 0.04
...
lights = find_computer_lights(bridge)
...
print ("The sun has set. Bedtime is at: %s, in %s seconds" % (bedtime, fadetime))
